...
#
# $Id: trowser.pod,v 1.2 2008/07/01 20:40:18 tom Exp $
# ---------------------------------------------------------------------------
...
text lines in the main window. You can not only use search patterns
to add text, but also add manually selected text lines via the
C<i> key binding or remove individual lines again.  Additionally you
can also use bookmarks in the search result window. So you effectively
can navigate the text on three levels.
...
be accessed with the cursor up/down keys like in vim.
...
C<:nohlsearch> in vim)  Note this does not disable highlighting in
subsequent searches.
...
Toggle line-wrap for text in the main window.
